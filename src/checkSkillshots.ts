...
async function main() {
  ...
  info('--findings--');
  info(JSON.stringify(findings, null, 2));
  info('--summary--');
  info(`${findings.length} finding(s); ${findings.filter(f => f.severity === 'strict').length} strict, ${findings.filter(f => f.severity === 'warn').length} warn`);

  await browser.close();
  process.exit(findings.filter(f => f.severity === 'strict').length > 0 ? 1 : 0);
}
main().catch(e => { error(e); process.exit(2); });
