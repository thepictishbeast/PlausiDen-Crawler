...
import type { Report, CapturedEvent } from './types.js';

export { type Report, type CapturedEvent };
...
export interface Diff {
  ...
  /**
   * font-loading findings new in this run vs prior. Warn-only:
   * @font-face rule with no font-display / font-display:block /
   * font-display:auto. T76.
   */
...
export function renderPositiveSignal(report: Report, diff: Diff): string {
  ...
  const axes: { name: string; total: number; news: number; strictNews: number }[] = [
    ...
    {
      // T76 (Crawler): font-loading — @font-face without
